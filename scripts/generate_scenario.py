...
def generate_scenario(runtime_fn, features_fn, cutoff):
    ...
    description = {"scenario_id": "data_test",
    ...
    with open(runtime_fn, "r") as fp:
        ...
        for line in fp:
            line = line.replace("\n","").split(",")
            inst = line[0]
            D=copy.deepcopy(description["algorithms_deterministic"])
            for algo, perf in zip(D, line[1:]):
    ...
    fs_data = [[inst, "1", stat] for inst,stat in status.items()]
    fs_attributes = [
                     ["instance_id", "STRING"],
                     ["repetition", "NUMERIC"],
